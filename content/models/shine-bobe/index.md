---
title: "eMOVING Shine Bobe"
description: "免駕照 自由行動不受限"

# 0. 頁首輪播圖 (新增此區塊，統一使用 .png)
hero_slides_pc:
  - "hero_1_pc.png"
  - "hero_2_pc.png"
  - "hero_3_pc.png"

hero_slides_mb:
  - "hero_1_mb.png"
  - "hero_2_mb.png"
  - "hero_3_mb.png"


# 1. 精彩試乘影片
videos:
  - id: "y8b_TQ8S2Q4"
    title: "全台第一家百貨公司菜市場?! 兩間隱藏台中巷弄60年必買人氣伴手禮 小菜一日採買 ft.eMOVING｜乾杯與小菜的日常"
  - id: "EmcQ7yWLL4w"
    title: "**獨家測試**emoving shine 續航力實測 台74甲線 大坡度路線 超硬 不開玩笑 花了一整天才測完"
  - id: "rYhtss5zxBw"
    title: "輕巧可愛的微型電動車🛵Emoving BOBE＆SHIN"


# 網紅推薦 IG 貼文嵌入 (新增區塊)
ig_posts:
  - url: "https://www.instagram.com/p/CTNC_KwpTfj/?utm_source=ig_embed&ig_rid=AjcciuoZIjxywZovYxr5vvF"
  - url: "https://www.instagram.com/p/CUpdZK7v9UX/?utm_source=ig_embed&ig_rid=Af6VIr1OVyU-mFzatiuzbll"



# 2. 車款特色 (已改為 .png)
features:
  - title: "微型電動二輪 免考駕照"
    desc: "台灣製造，安全性最高！年滿14歲 就能輕鬆騎"
    image: "feature_1.png"
  - title: "Shine v.s Bobe - 車頭"
    desc: "Shine 為流線造型，頭燈在下方。Bobe 為復古造型，頭燈在上方。"
    image: "feature_2.png"
  - title: "Shine v.s Bobe - 置物空間"
    desc: "Shine 比 Bobe 大。"
    image: "feature_3.png"  
  - title: "Shine v.s Bobe - 開啟車廂的方式"
    desc: "Shine 可於鑰匙孔開啟，Bobe 要拔鑰匙插側邊鑰匙孔。"
    image: "feature_4.png"
  - title: "Shine v.s Bobe - 坐墊/座高"
    desc: "Shine 的座高較 Bobe 低，Bobe 的座墊比較接近傳統油車座墊。"
    image: "feature_5.png"
 


# 3. 詳細規格數據表注意事項（可依車款新增多條）
specs_notes:
  - "* 規格配備以實車為準，本公司保留修改權利。"
  - "註:<br>

   1.充滿一次電池約$1.87元(48*13/1000)*$3/度)，最高續航里程為前題下，平均$1元可騎乘21.4km。<br>

   2. 最高續航力為實驗室測試值，模擬55kg載重於定速20km/h下的最高行駛距離，實際續航力因載重條件、個人騎乘習慣、騎乘路況與天候狀況的不同而有所差異。<br>

   3. 保留加裝第二顆電池空間，可大幅提升續航力。數值有些微差異，故統一以概略值標示。<br>

   4. 菜籃僅示意，購買商品以專門店現場實物為準。<br>

   5. 電池保固2年，整車保固1年，詳細保固條件請參考車主手冊。<br>

   6.首家通過電動輔助自行車及電動自行車(改稱微型電動二輪車)安全檢測基準審查，法規項目：三之一、電子控制裝置之合格廠牌。<br>

   7. 隨車檢附「微型電動案輪車型式安全審驗合格證明及完成車四面照片」放置於置物箱內。<br>

   8. 不得擅自改裝 車輛規格及電子控制裝置含增、減或變更車輛裝置。<br>

   車主車輛使用常見問題請參閱：<a href='https://www.e-moving.com.tw/QA' target='_blank'>https://www.e-moving.com.tw/QA</a>。"

specs_tables:
- title: "SHINE"
  specs:
  - label: "長 x 寬 x 高"
    value: "1,560 X 655 X 1,015 mm"
  - label: "重量(含電池)"
    value: "約 40kg"
  - label: "安全載重"
    value: "100kg"
  - label: "最高續航力"
    value: "40 km"
  - label: "爬坡"
    value: "16% (9°)"
  - label: "極速"
    value: "< 25km/h"
  - label: "輪胎尺寸"
    value: "皮10吋鋁圈"
  - label: "形式"
    value: "輪外馬達"
  - label: "最大功率"
    value: "800W"
  - label: "電池形式"
    value: "可抽取式鋰離子電池"
  - label: "電池重量"
    value: "5.6kg"
  - label: "電壓．容量"
    value: "48V．13AH"
  - label: "充電方式"
    value: "外接充電器"
  - label: "充電能源形式"
    value: "AC110V．220V"
  - label: "方向燈 / 尾燈 / 儀表"
    value: "LED玫瑰金方向燈 / 反射式LED尾燈 / 彩色液晶顯示儀表板"
  - label: "中控鎖"
    value: "三合一 (啟動+龍頭鎖+開車箱)"
  - label: "貼心配備"
    value: "防暴衝裝置 / 前膝置物盒(選配) / 前掛勾 / 側駐車架 / 中駐車架"
  - label: "能源效率"
    value: "21.4km/$1(註1"
  - label: "電池保固"
    value: "2年"
  - label: "年耗電量(度)"
    value: "189 度"


- title: "BOBE"
  specs:
  - label: "長 x 寬 x 高"
    value: "1,560 X 655 X 1,015 mm"
  - label: "重量(含電池)"
    value: "約 40kg"
  - label: "安全載重"
    value: "100kg"
  - label: "最高續航力"
    value: "40 km"
  - label: "爬坡"
    value: "16% (9°)"
  - label: "極速"
    value: "< 25km/h"
  - label: "輪胎尺寸"
    value: "皮10吋鋁圈"
  - label: "形式"
    value: "輪外馬達"
  - label: "最大功率"
    value: "800W"
  - label: "電池形式"
    value: "可抽取式鋰離子電池"
  - label: "電池重量"
    value: "5.6kg"
  - label: "電壓．容量"
    value: "48V．13AH"
  - label: "充電方式"
    value: "外接充電器"
  - label: "充電能源形式"
    value: "AC110V．220V"
  - label: "方向燈 / 尾燈 / 儀表"
    value: "復古鹵素方向燈 / 復古方形鹵素尾燈 / 彩色液晶顯示儀表板"
  - label: "中控鎖"
    value: "二合一 (啟動+龍頭鎖)"
  - label: "貼心配備"
    value: "防暴衝裝置 / 前膝置物盒 / 前掛勾 / 側駐車架 / 中駐車架"
  - label: "能源效率"
    value: "21.4km/$1(註1"
  - label: "電池保固"
    value: "2年"
  - label: "年耗電量(度)"
    value: "189 度"


# 4. 媒體報導觀點 (已改為 .png)
reports:
  - source: "地球黃金線"
    author: "謝明翰"
    image: "report_1.png"
    summary: "eMOVING主打女性族群的微型電動二輪車Shine，2025年全新推出「羅蘭紫」車色，以霧面質感搭配手繪花型車貼，增添浪漫且富有層次的視覺氛圍，低調中展現獨特氣質，為日常代步增添一抹優雅。新車上市搶先促銷，. . . . "
    link: "https://cars.tvbs.com.tw/car-news/248528"

  - source: "Car Stuff"
    author: "Jason Hu"
    image: "report_2.png"
    summary: "微型電動二輪車掛牌上路，法規推行首月掛牌數開盤，eMOVING以2,735台一舉奪下領牌冠軍，市佔率達13.4%(註)。微型電動二輪車以免駕照、免電池月租、車身輕巧且容易保養等特性，近年來成為民眾「汰油轉電」之首選車種，2022年11月30日新法上路，電動自行車更名為微型電動二輪車，正式領牌納管，為鼓勵車主合法上路，. . . . "
    link: "https://www.carstuff.com.tw/motocycle/item/37526-emoving-6-000.html"

  - source: "Car Stuff"
    author: "陳奕宏"
    image: "report_3.png"
    summary: "對於無法領取駕照、不得騎乘輕型機車的族群，如年紀較大的婆婆媽媽、新住民，或者是距離學校較遠卻尚未屆滿考照年齡的族群，動力輸出並未達到電動機車的水準，卻有如同機車般便利性的電動自行車變成為了最佳的選擇。於是中華汽車於2013年便搶先推出e-moving Bobe (保庇) 電動自行車，以擴大自身在兩輪電動車的產品陣容。. . . . "
    link: "https://www.carstuff.com.tw/motocycle/item/17249-e-moving-bobe.html"

# 5. 專屬原廠配件 (已改為 .png)
accessories:
  - title: "Shine前置物盒"
    image: "acc_1.png"
  - title: "好裝菜籃"
    image: "acc_2.png"
  - title: "後置物架+後置物箱"
    image: "acc_3.png"
  - title: "鋁合金手機架"
    image: "acc_4.png"


# 6. 常見問題解答
faq:
  - question: "Q. 什麼是微型電動二輪車？騎乘 eMOVING 需要駕照嗎？"
    answer: |
      A. 不需要駕照。 微型電動二輪車（簡稱微電車）是指經型式審驗合格，以電力為能量，最大行駛速率在每小時 25 公里以下，且車重（不含電池）在 40 公斤以下之二輪車輛。
      只要年滿 14 歲，不需考取駕照即可合法騎乘 eMOVING 微電車。 
  - question: "Q. 騎乘 eMOVING 微電車有哪些交通法規與罰則需要注意？"
    answer: |
      A. 騎乘 eMOVING 微電車上路時，必須嚴格遵守以下四個核心規定，否則將面臨罰鍰：
      年齡限制： 必須年滿 14 歲才可騎乘。
      配戴安全帽： 騎乘時必須配戴合格的機車用安全帽（違者罰 300 元）。
      嚴禁載人： 微電車依法全時段禁止雙載（違者罰 300 至 600 元）。
      行速限制： 法定最高時速為 25 km/h，嚴禁擅自改裝擴大速限（改裝者最高可罰 5,400 元）。
  - question: "Q. eMOVING 微電車需要掛牌、投保強制險嗎？"
    answer: |
      A.需要。 台灣自 2022 年 11 月起已立法通過，所有微型電動二輪車不論新舊，都必須登記、領用並懸掛牌照，且必須投保強制汽車責任保險才可上路。
      購買 eMOVING 新車時，專賣店通常會協助辦理相關掛牌與投保手續。 
  - question: "Q. eMOVING Shine 與 Bobe 的主要規格與續航力有什麼差別？"
    answer: |
      A.兩款車都採用抽取式鋰離子電池，車重都控制在 40 公斤左右（不含電池），非常輕巧好牽。 詳細差異可往上滑查看
  - question: "Q. eMOVING 的電池要怎麼充電？充飽電要花多少錢？"
    answer: |
      A. eMOVING 微電車採用抽取式鋰離子電池，重量僅約 5.3 至 5.6 公斤。
      充電方式： 支援「家充」與「車充」雙模式。你可以把電池提回客廳用一般家用 AC 110V 插座充電，也可以直接在車上插線充電。
      超級省錢： 每次充滿電約僅需 0.62 度電（電費約 1.87 元）。換算下來，平均花費 1 元台幣就能騎乘高達 21.4 公里，比加油或月租費資費划算非常多。
  - question: "Q. eMOVING 的微電車電池有保固嗎？不騎時該怎麼保養？"
    answer: |
      A. 原廠保固： eMOVING 提供鋰電池最高 2年安心保固（整車保固 1 年），詳細條款依車主手冊為準。
      電池保養技巧： 若長時間不騎乘（1 個月以上），建議先將電池充飽，並將電池從車輛拔下或關閉電源。每個月定期檢查並充電一次，避免電池因細胞自放電導致過度放電而損壞。

# 電子型錄 (對應 static/catalog/ez1.pdf)
catalog_url: "catalog/em25.pdf"

# 車主手冊 (假設 PDF 放在 static/manual/ 資料夾內)
manual_url: "manual/em25.pdf"

# ==========================================
# 視差滾動區塊專用獨立設定
# ==========================================

# 1. 視差車色去背圖、名稱與背景色配對
parallax_slides:
  - image: "bobe-blue.png"
    color_name: "BOBE 都會藍"
    bg_color: "#69D9FF"
  - image: "bobe-white.png"
    color_name: "BOBE 簡約白"
    bg_color: "#F7F7F7"
  - image: "shine-purple.png"
    color_name: "SHINE 羅蘭紫"
    bg_color: "#CAC2FF"
  - image: "shine-gold.png"
    color_name: "SHINE 太妃金"
    bg_color: "#E7C16E"  
  - image: "shine-white.png"
    color_name: "SHINE 精靈白"
    bg_color: "#F7F7F7"  
  - image: "shine-blue.png"
    color_name: "SHINE 天空藍"
    bg_color: "#69D9FF"      

# 2. 車款特色列表 (一特色對應一張專屬背景圖)
parallax_features:
  - number: "01"
    title: "可抽取式電池"
    desc: "住公寓大樓充電免擔心，可提進家裡用110V插座充電。"
    image: "feature-bg-01.png"
    color_class: "text-yellow-400"
    badge_bg: "bg-yellow-400"

  - number: "02"
    title: "彩色液晶儀表"
    desc: "簡約直覺，電量剩多少看得到。"
    image: "feature-bg-02.png"
    color_class: "text-cyan-400"
    badge_bg: "bg-cyan-400"

  - number: "03"
    title: "寬敞置腳空間"
    desc: "連黃金獵犬也能上去，輕鬆載著出發去運動。"
    image: "feature-bg-03.png"
    color_class: "text-emerald-400"
    badge_bg: "bg-emerald-400"

# 視差固定背景圖片
fixed_picture_pc: "fixed_bg_desktop.png" # 電腦版 (橫圖, 如 1920x1080)
fixed_picture_mb: "fixed_bg_mobile.png"  # 手機版 (直圖, 如 800x1200)

---